...
def scrape_model(xlfile_name, inputsheet, outputsheet, clearsheet, clearsheet_items):
    try:
...
# Code replaced with merge above
#    for item in clearsheet_items:
#        try:    
#            if clearsheet_items[item] in dfclearsheet.index:
#                clearsheet_var.update({item: dfclearsheet.loc[clearsheet_var[item], 1]})
#            else: clearsheet_var.update({item: ''})
#        except:
#            clearsheet_var.update({item: ''})
           
    return finput, foutput, dffinput, dfoutput, finput_timestamp, clearsheet_metadata

for file in xlsxfiles:  # For each model file we are working with...
    finput, foutput, dffinput, dfoutput, finput_timestamp, clearsheet_metadata = scrape_model(file, inputsheet, outputsheet, clearsheet, clearsheet_items)  # ...get the list of BON codes in the f_input and f_output sheets
